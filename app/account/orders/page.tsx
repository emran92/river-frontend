      ...
      {loading ? (
        ...
        <>
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            {/* Table header */}
            <div className="hidden sm:grid grid-cols-[1fr_120px_100px_90px_40px] gap-4 px-5 py-3 bg-gray-50 border-b border-gray-100 text-xs font-semibold text-gray-500 uppercase tracking-wide">
              <span>Order</span>
              <span>Date</span>
              <span>Total</span>
              <span>Status</span>
            ...
            <div className="divide-y divide-gray-50">
              {orders.map((order) => {
                ...
                return (
                  <div key={order.id} className="grid grid-cols-1 sm:grid-cols-[1fr_120px_100px_90px_40px] gap-3 sm:gap-4 px-5 py-4 hover:bg-gray-50/60 transition-colors">
                    {/* Order info */}
                    <div>
                      <p className="font-semibold text-sm text-gray-900">#{order.order_number}</p>
                      <p className="text-xs text-gray-400 mt-0.5 sm:hidden">{formatDate(order.created_at)}</p>
                    ...
                    <div className="flex sm:items-center justify-between sm:justify-start">
                      <span className="font-semibold text-sm text-gray-900">{formatBDT(Number(order.total))}</span>
                      {/* Status on mobile */}
                    ...
                    <div className="hidden sm:flex items-center justify-end">
                      <Link
                        ...
                        className="text-gray-400 hover:text-river-blue transition-colors"
          ...
          {lastPage > 1 && (
            <div className="flex items-center justify-center gap-2">
              <button
                ...
                disabled={page === 1}
                className="flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-gray-600 border border-gray-200 bg-white rounded-xl hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed transition-colors shadow-sm"
              >
