      ...
      <div className="max-w-[1280px] mx-auto px-4">
        <div className="flex items-center h-11">
          ...
          <div className="hidden md:flex items-center flex-1 gap-4 ml-6">
            {NAV_LINKS.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="px-3 h-11 flex items-center text-sm text-gray-700 dark:text-gray-200 hover:text-blue-600 dark:hover:text-blue-400 transition-colors whitespace-nowrap"
              >
                {link.label}
              </Link>
            ))}
          ...
          <div className="hidden md:flex items-center gap-2 ml-auto text-sm text-gray-600">
            ...
            <span className="text-xs text-gray-500 dark:text-gray-400">Customer Support</span>
          </div>

          {/* Mobile menu toggle */}
          <button
            className="md:hidden ml-auto p-2 text-gray-600 dark:text-gray-300"
        ...
        {/* Mobile nav */}
        {mobileOpen && (
          <div className="md:hidden border-t border-gray-100 dark:border-gray-700 py-2">
            {NAV_LINKS.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="block px-4 py-2.5 text-sm text-gray-700 dark:text-gray-200 hover:bg-river-blue/10 hover:text-blue-600 dark:hover:text-blue-400"
